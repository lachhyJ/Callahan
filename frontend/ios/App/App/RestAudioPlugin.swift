...
public class RestAudioPlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    private func arm(for endAt: Date, force: Bool) -> Bool {
        ...
        guard endAt.timeIntervalSinceNow > 0.25 else {
            ...
            // due. On resume, JS reconciles from native state and can re-schedule
            // a rest that already beeped while backgrounded; arming that played
            // the beep again, and playImmediately ducks, so the music dipped every
            // time the app was opened after a rest ended.

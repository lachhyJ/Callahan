...
- **2026-08-06** — Rest-timer signature detail (a depleting progress ring/bar replacing the plain countdown number + flat buttons) is a deliberate deferral, not a rejection. `/finalize` should keep naming it as the one open craft-intent gap, not treat it as resolved or drop it — it's real backlog, just not now. Revisit when there's dedicated time for a proper build (SVG/CSS progress synced to `remainingSeconds`/`totalSeconds`, ~30-45 min), not squeezed in as a quick add-on.
